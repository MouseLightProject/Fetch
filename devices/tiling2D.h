...
namespace device {
  ...
  public:
    ...
    bool     nextSearchPosition(int iplane, int ntimes, Vector3f &pos,TileSearchContext2D **ctx);     ///< *ctx should be NULL on the first call.  It will be internally managed.
    void     tileSearchCleanup(TileSearchContext2D *ctx);
 ...
 /*struct tile_search_parent_t;
  ...
  { StageTiling2D *tiling;
    ...
    TileSearchContext2D(StageTiling2D *t,int radius=0);
    ~TileSearchContext2D();
    void sync();
