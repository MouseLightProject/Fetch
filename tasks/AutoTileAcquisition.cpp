...
Error:
      ...
      */
      ...
      { unsigned eflag=0; //success
		...
		fetch::device::StageTiling * tiling = dc->stage()->tiling();
        ...
        { 
		  ...
		  { 
			...
			if(previousPosInLattice != currentPosInLattice)
		      dc->stage()->tiling()->useCurrentDoneTilesAsCurrentExplorableTiles(currentPosInLattice);
		  }
		  else dc->stage()->tiling()->useCurrentDoneTilesAsNextExplorableTiles(); //DGA: After imaging tiles, set the next explorable tiles equal to the current done tiles
        }
