...
#define CHK(expr) \
  ...
  */
  ...
  {
    float cx,cy,cz,vx,vy,vz,ax,ay,bx,by,bz,v,dz,thick, manual_z_offset;
    // get current pos,vel
	...
	manual_z_offset = dc->vibratome()->manualVerticalOffset();
    CHK( (v = dc->vibratome()->feed_vel_mm_p_s())>0.0); // must be non-zero

    // Move to the start of the cut
    bz = cz-dz+thick + (manual_z_offset);								// cut z position = Current Z - delta Z offset + requested slice thickness ( + offset)
