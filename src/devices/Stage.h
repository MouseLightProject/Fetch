...
namespace fetch {
  ...
  bool operator==(const cfg::device::C884StageController& a, const cfg::device::C884StageController& b);
  ...
  namespace device {
    ...
    public:
      ...
      inline void  getSafeVelocity(float *vx, float *vy, float *vz) { getSafeVelocityX(vx); getSafeVelocityY(vy); getSafeVelocityZ(vz); debug("(%s:%d): Velocity set to %f %f %f"ENDL, __FILE__, __LINE__, *vx, *vy, *vz); } //DGA: Set safe vx, vy and vz velocities
    ...
    class C884Stage :public StageBase<cfg::device::C884StageController>
    {
    public:
      C884Stage(Agent *agent);
      C884Stage(Agent *agent, Config *cfg);
      virtual ~C884Stage();
